...
class ReportsGenerator:
    ...
    def _get_sentences_embeddings(self, original_sentences):
        ...
        if not self.force_huggingface_embeddings:
            try:
                embeddings = self._get_embeddings_deep_inference_models(cleaned_text)
            except Exception as e:
                if self.n_warnings == 0:
                    warnings.warn(
                        "Embedding generattion using DEEP inference models generated an error. Switching to HuggigFace Embedding model."
                    )
                    print("error", e)
                    self.n_warnings += 1
    ...
    def _get_embeddings_deep_inference_models(self, cleaned_text: List[str]):
        ...
        inputs = pd.DataFrame(cleaned_text, columns=["excerpt"])
        ...
        # kw for embeddings
        inputs["output_backbone_embeddings"] = True
        inputs["pooling_type"] = "['mean_pooling']"
        inputs["finetuned_task"] = "['first_level_tags']"
        inputs["embeddings_return_type"] = "array"

        backbone_inputs_json = inputs.to_json(orient="split")

        response = self.client.invoke_endpoint(
            EndpointName=self.deployed_endpoint_name,
            Body=backbone_inputs_json,
            ContentType="application/json; format=pandas-split",
        )
        output = response["Body"].read().decode("ascii")

        output = literal_eval(output)["output_backbone"]
    ...
    def _get_embeddings_huggingface_models(self, cleaned_text: List[str]):
        ...
        if not hasattr(self, "embeddings_tokenizer"):
    ...
    def _summarize_one_cluster(
        ...
        cosine_similarity_matrix = euclidean_distances(
    ...
    def __call__(
        ...
        Args:
            ...
            - force_huggingface_embeddings: bool:
                If True, prioritize the usage of DEEP deployed embedding models
                If False, use HuggingFace embedding model
            - deployed_endpoint_name: deployed endpoint link.
        """
        ...
        self.force_huggingface_embeddings = force_huggingface_embeddings
        self.deployed_endpoint_name = deployed_endpoint_name

        if (
            self.deployed_endpoint_name is None
            and not self.force_huggingface_embeddings
        ):
            warnings.warn(
                "arg 'force_huggingface_embeddings' is False but no 'deployed_endpoint_name' has been provided. Please provide a valid link for the 'deployed_endpoint_name'. Switching to HuggingFace embedding model usage for now."
            )
            self.force_huggingface_embeddings = True
